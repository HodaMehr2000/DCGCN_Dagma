...
class CASECausalPred(BaseSupervisor):
    ...
    def load_adj_mx(self):
        ...
        # return adj_mat
        # مسیر اولویت‌دار: زیر dataset_dir (مثلاً data/METR-LA/N20/sensor_graph/adj_mx.pkl)
        ds = self._data_kwargs.get('dataset_dir', '')
        p1 = os.path.join(self.base_dir, ds, 'sensor_graph', 'adj_mx.pkl')
        if os.path.exists(p1):
            adj_mat = load_graph(p1)
        else:
            # مسیر fallback: data/sensor_graph/adj_mx.pkl
        ...
        # در صورت بزرگتر بودن از num_nodes، برش بزن
        if adj_mat.shape[0] > self.num_nodes:
            adj_mat = adj_mat[:self.num_nodes, :self.num_nodes]
        return adj_mat
